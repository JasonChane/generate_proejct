...
@implementation LionNonValue

+ (LionNonValue *)value
{
    static LionNonValue * __value = nil;
    
    if ( nil == __value )
    {
        __value = [[LionNonValue alloc] init];
    }
...
@implementation LionActiveObject : NSObject

- (NSString *)description
{
    NSMutableString * desc = [NSMutableString string];
    
    Class rootClass = [LionActiveObject class];
...
@interface NSObject(LionActiveObjectPrivate)
...
@implementation NSObject(LionActiveObject)
...
+ (void)mapRelationForClass:(Class)rootClass
{
    ...
    for ( Class clazzType = self; clazzType != rootClass; )
    {
        ...
        for ( NSUInteger i = 0; i < propertyCount; i++ )
        {
            ...
            {
                NSString * attrClassName = [LionTypeEncoding classNameOfAttribute:attr];
                if ( attrClassName )
                {
        ...
        isDictionary:(BOOL)isDictionary
{
    ...
    if ( propertySet )
    {
        ...
        if ( property )
        {
            ...
            {
                [property setObject:[LionNonValue value] forKey:@"value"];
            }
