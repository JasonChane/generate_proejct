...
@implementation LionActiveRecord
...
+ (BOOL)autoLoad
{
    ...
    NSArray * availableClasses = [LionRuntime allSubClassesOf:[LionActiveRecord class]];
    ...
    for ( Class classType in availableClasses )
    {
        ...
        [LionActiveBuilder buildTableFor:classType untilRootClass:[LionActiveRecord class]];
...
+ (void)mapRelation
{
    [self mapRelationForClass:[LionActiveRecord class]];
}

+ (LionDatabase *)DB
{
    [LionActiveBuilder buildTableFor:self untilRootClass:[LionActiveRecord class]];
    ...
    if ( NO == __enterred )
    {
        ...
        if ( object == self )	// sync property to JSON
        {
            ...
            if ( value && NO == [value isKindOfClass:[LionNonValue class]] )
            {
                ...
                else if ( LionTypeEncoding.NSARRAY == type.intValue )
                {
                    ...
                    if ( array && array.count )
                    {
                        ...
                        if ( className )
                        {
                            ...
                            if ( class )
                            {
                                ...
                                if ( [class isSubclassOfClass:[LionActiveRecord class]] )
                                {
                                    for ( NSObject * elem in array )
                                    {
                                        if ( NO == [elem isKindOfClass:[LionActiveRecord class]] )
                                            continue;
                                        
                                        NSDictionary * dict = [(LionActiveRecord *)elem JSON];
                                        if ( dict )
                                        {
                ...
                else if ( LionTypeEncoding.NSDICTIONARY == type.intValue )
                {
                    value = [value asNSMutableDictionary];	// convert to NSMutableDictionary
                }
                else if ( LionTypeEncoding.OBJECT == type.intValue )
                {
                        ...
                        if ( className )
                        {
                            ...
                            if ( class )
                            {
                                ...
                                if ( [class isSubclassOfClass:[LionActiveRecord class]] )
                                {
                                    for ( NSObject * elem in array )
                                    {
                                        LionActiveRecord * record = [class recordWithObject:elem];
                                        if ( record )
                                        {
                ...
                else if ( LionTypeEncoding.OBJECT == type.intValue )
                {
                    ...
                    if ( className )
                    {
                        ...
                        if ( class )
                        {
                            if ( [class isSubclassOfClass:[LionActiveRecord class]] )
                            {
                                value = [class recordWithObject:value];			// convert value to LionActiveRecord
                            }
                            else
                            {
...
- (void)initSelf
{
    [LionActiveBuilder buildTableFor:[self class] untilRootClass:[LionActiveRecord class]];
    ...
    if ( propertySet && propertySet.count )
    {
        for ( NSString * key in propertySet.allKeys )
        {
            ...
            else
            {
                ...
                if ( LionTypeEncoding.OBJECT == type.intValue )
                {
                    NSString * className = [property objectForKey:@"className"];
                    if ( className )
                    {
                ...
                if ( db.succeed )
                {
                    ...
                    if ( dict && [dict isKindOfClass:[NSDictionary class]] )
                    {
                        ...
                        for ( NSString * key in dict.allKeys )
                        {
                            ...
                            {
                                NSString * propertyName = [LionActiveRecord identifierForFieldName:key];
...
- (id)initWithObject:(NSObject *)object
{
    ...
    if ( self )
    {
        ...
        else if ( [object isKindOfClass:[LionActiveRecord class]] )
        {
            [self setDictionary:((LionActiveRecord *)object).JSON];
        }
        else
        {
...
+ (id)record:(NSObject *)otherObject
{
    ...
    else if ( [otherObject isKindOfClass:[NSArray class]] )
    {
        ...
        {
            LionActiveRecord * record = [[self class] record:obj];
            if ( record )
            {
                [results addObject:record];
            }
...
- (void)setPropertiesFrom:(NSDictionary *)dict
{
    ...
    for ( NSString * key in propertySet.allKeys )
    {
        ...
        if ( nil == value )
        {
            ...
            if ( [value isKindOfClass:[LionNonValue class]] || [value isKindOfClass:[NSNull class]] )
            {
                value = nil;
            }
        }
...
- (NSDictionary *)JSON
{
    ...
    return [self objectToDictionaryUntilRootClass:[LionActiveRecord class]];
#endif	// #if __JSON_SERIALIZATION__
}
...
#else	// #if __JSON_SERIALIZATION__
    return [self objectToDataUntilRootClass:[LionActiveRecord class]];
#endif	// #if __JSON_SERIALIZATION__
}
...
#else	// #if __JSON_SERIALIZATION__
    NSString * json = [self objectToStringUntilRootClass:[LionActiveRecord class]];
#endif	// #if __JSON_SERIALIZATION__
...
- (BOOL)insert
{
    ...
    for ( NSString * key in propertySet.allKeys )
    {
        ...
        if ( LionTypeEncoding.OBJECT == type.intValue )
        {
            ...
            if ( value && [value isKindOfClass:[LionActiveRecord class]] )
            {
                LionActiveRecord * record = (LionActiveRecord *)value;
    ...
    for ( NSString * key in propertySet.allKeys )
    {
        ...
        if ( value )
        {
            ...
            {
                if ( [value isKindOfClass:[LionActiveRecord class]] )
                {
                    LionActiveRecord * record = (LionActiveRecord *)value;
...
@interface __TestLocation : LionActiveRecord
@property (nonatomic, retain) NSNumber *		lid;
@property (nonatomic, retain) NSNumber *		lat;
@property (nonatomic, retain) NSNumber *		lon;
@end

@interface __TestRecord : LionActiveRecord
...
TEST_CASE( LionActiveRecord )
{
    TIMES( 3 )
    {
        HERE( "open close DB", {
            ...
            [LionDatabase openSharedDatabase:@"ar2"];
    ...
    TIMES( 3 )
    {
        ...
        HERE( "Query records", {
            __TestRecord2.DB.GET_RECORDS();	// Results are LionActiveRecord
