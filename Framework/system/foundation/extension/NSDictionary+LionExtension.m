...
@implementation NSDictionary(LionExtension)
...
- (id)objectForClass:(Class)clazz
{
    ...
    for ( Class clazzType = clazz; clazzType != [NSObject class]; )
    {
        ...
        for ( NSUInteger i = 0; i < propertyCount; i++ )
        {
            ...
            BOOL readonly = [LionTypeEncoding isReadOnly:attr];
            if ( readonly )
                continue;
            ...
            if ( tempValue )
            {
                ...
                {
                    NSString * className = [LionTypeEncoding classNameOfAttribute:attr];
...
@implementation NSMutableDictionary(LionExtension)
...
TEST_CASE( NSDictionary_LionExtension )
{
}
TEST_CASE_END
