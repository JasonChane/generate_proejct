...
@implementation NSObject(LionSocket)
...
@interface LionSocket()
{
    ...
    LionSocketBlock			_whenUpdate;
    LionSocketBlock			_whenConnecting;
    LionSocketBlock			_whenConnected;
    LionSocketBlock			_whenAccepting;
    LionSocketBlock			_whenAccepted;
    LionSocketBlock			_whenDisconnecting;
    LionSocketBlock			_whenDisconnected;
    LionSocketBlock			_whenStopping;
    LionSocketBlock			_whenStopped;
    LionSocketBlock			_whenListenning;
    LionSocketBlock			_whenAcceptable;
    LionSocketBlock			_whenWritable;
    LionSocketBlock			_whenReadable;
    LionSocketBlock			_whenHeartBeat;
...
{
    LionSocket * socketObj = (LionSocket *)info;
    if ( nil == socketObj )
        return;
...
+ (LionSocket *)socket
{
    return [[[LionSocket alloc] init] autorelease];
}
...
{
    LionSocket * conn = [[LionSocket alloc] init];
    if ( nil == conn )
    {
        return nil;
    }
...
- (id)init
{
    ...
    if ( self )
    {
        ...
        _role = LionSocket.ROLE_CLIENT;
        _state = LionSocket.STATE_CREATED;
        _errorCode = 0;
...
- (BOOL)isClient
{
    return LionSocket.ROLE_CLIENT == _role ? YES : NO;
}

- (BOOL)isServer
{
    return LionSocket.ROLE_SERVER == _role ? YES : NO;
}

- (BOOL)isDaemon
{
    return LionSocket.ROLE_DAEMON == _role ? YES : NO;
}
...
- (BOOL)created
{
    return LionSocket.STATE_CREATED == _state ? YES : NO;
}

- (BOOL)connecting
{
    if ( LionSocket.ROLE_CLIENT != _role || LionSocket.ROLE_SERVER != _role )
    {
        return NO;
    }
    
    return LionSocket.STATE_CONNECTING == _state ? YES : NO;
}
...
- (BOOL)connected
{
    ...
    return LionSocket.STATE_CONNECTED == _state ? YES : NO;
}

- (BOOL)accepting
{
    if ( LionSocket.ROLE_SERVER != _role )
    {
        return NO;
    }
...
- (BOOL)disconnecting
{
    ...
    return LionSocket.STATE_DISCONNECTING == _state ? YES : NO;
}
...
- (BOOL)disconnected
{
    ...
    return LionSocket.STATE_DISCONNECTED == _state ? YES : NO;
}
...
- (BOOL)listenning
{
    if ( LionSocket.ROLE_DAEMON != _role )
    {
        return NO;
    }
...
- (BOOL)internalOpen:(int)sock
{
    ...
    else
    {
        ...
        if ( LionSocket.ROLE_DAEMON == self.role )
        {
            addr.sin_port = htons( self.port );
        }
        else if ( LionSocket.ROLE_CLIENT == self.role )
        {
            addr.sin_port = htonl( INADDR_ANY );
        }
        else if ( LionSocket.ROLE_SERVER == self.role )
        {
            addr.sin_port = htonl( INADDR_ANY );
        }
...
- (BOOL)internalAccept:(int)sock
{
    ...
    self.role	= LionSocket.ROLE_SERVER;
    self.host	= nil;	// TODO:
    self.port	= 0;	// TODO:
...
- (NSUInteger)internalRead
{
    ...
    {
        [self internalError:LionSocket.ERROR_CONNECTION_RESET];
        return 0;
    }
...
- (BOOL)listen:(NSUInteger)port
{
    ...
    self.role = LionSocket.ROLE_DAEMON;
    self.host = nil;
    self.port = port;
...
- (BOOL)connect:(NSString *)addr port:(NSUInteger)port
{
    ...
    self.role = LionSocket.ROLE_CLIENT;
...
- (LionSocket *)accept
{
    ...
    LionSocket *	conn = [LionSocket socket:sock];
    if ( conn )
    {
...
- (void)handleEventError:(NSInteger)error
{
    ...
    [self internalError:LionSocket.ERROR_UNKNOWN];
}
