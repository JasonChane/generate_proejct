...
@interface LionHTTPRequest2()
- (BOOL)checkIntegrity:(NSData *)data;
@end

#pragma mark -

@implementation LionHTTPRequest2
...
{
    return [[[LionHTTPRequest2 alloc] init] autorelease];
}

+ (LionHTTPRequest2 *)request:(NSData *)data
{
    LionHTTPRequest2 * req = [[[LionHTTPRequest2 alloc] init] autorelease];
    if ( req )
    {
...
{
    LionHTTPResponse2 * response = [LionHTTPResponse2 response];
    if ( response )
    {
...
- (NSString *)packHead
{
    ...
    if ( LionHTTPMethod_GET == self.method )
    {
        [headLine appendString:@"GET"];
    }
    else if ( LionHTTPMethod_HEAD == self.method )
    {
        [headLine appendString:@"HEAD"];
    }
    else if ( LionHTTPMethod_POST == self.method )
    {
        [headLine appendString:@"POST"];
    }
    else if ( LionHTTPMethod_PUT == self.method )
    {
        [headLine appendString:@"PUT"];
    }
    else if ( LionHTTPMethod_DELETE == self.method )
    {
